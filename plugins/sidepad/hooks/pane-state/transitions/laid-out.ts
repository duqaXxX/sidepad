...
/**
 * The formatted page kept where the reader was after a width change: a row is a width's own, so the
...
function keptOnItsBlock(before: PaneState, after: PaneState): PaneState {
  const view = formattedViewOf(after);
  const was = formattedPageOf(before);
  const now = formattedPageOf(after);
  const file = after.file;

  if (!view || !file || !was || !now) {
    return after;
  }
...
/**
 * The state for a drawing's body size: windows kept inside it, and on a new width the formatted
