 ...
 *
 * LIMIT(wheel-reversal-drops-tick): Claude Code 2.1.280 sends no `ui.scroll` for the first wheel tick after the wheel changes
 * direction (#38), so that tick moves nothing.
 *
 * LIMIT(home-end-move-one-page): Home and End arrive as `by` the engine's own tree rows, and that tree always fits the body,
