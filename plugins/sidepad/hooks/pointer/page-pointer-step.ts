 ...
 * relative to the Client's own content, so the page row is the window's first row plus it.
 *
...
export function pagePointerStep(drag: CodeDrag, event: ClientPointerEvent, window: PageWindow): PagePointerStep {
  const visible = Math.max(1, window.rowCount);
  const row = window.firstRow + Math.max(0, Math.min(event.y, visible - 1));
