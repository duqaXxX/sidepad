...
- `plugins/sidepad/hooks/surfaces/markdown-page-view.tsx`, `rowOf`: on a terminal Claude Code does not take for one that opens hyperlinks, a `Link` draws its URL after its text, which the row was not laid out to hold: the row is cut at the page's edge and loses its end (Claude Code 2.1.278). No declaration says which case holds.
