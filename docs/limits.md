...
- `.github/scripts/live/screen.ts`, `codeRowsOf`: Claude Code 2.1.280 numbers no trailing blank line of a `Code` (#39), so a window ending on a selected blank line draws its row as the marker alone. Such a row right below a code row is read as the next line; an unselected one draws nothing and is not read at all.
